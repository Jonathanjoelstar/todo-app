	...
	<div class="w-1/8 flex relative">
		...
		<Popover class="relative">
			...
			<PopoverPanel data-popover="popover-bottom-start" data-popover-placement="bottom-start" as="Listbox">
				<Listbox
					class="block absolute -right-0 top-10 z-10 p-4 font-sans text-sm font-normal break-words whitespace-normal bg-white rounded-lg shadow-lg w-max border-blue-gray-50 text-blue-gray-500 shadow-blue-gray-500/10 h-60 w-60"
					...
					<ListboxOptions class="max-h-40 w-full overflow-auto scrollbar" static>
						<ListboxOption as="template" v-for="tag in tags" :key="tag.id" :value="tag"
							v-slot="{ selected }">
